...
while i < len(cds):
    ...
    for j in range(no_of_tabs):
        ...
        driver.switch_to.window(driver.window_handles[j])
        try:
            element = driver.find_element(By.CSS_SELECTOR, 'table.table:nth-child(3) > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(4)')
            if element.get_attribute('innerHTML').find('FATTY ACID BINDING PROTEIN HOMOLOG') != -1:
                l.append(unique_code[i])
            print(l)
            print(unique_code)
